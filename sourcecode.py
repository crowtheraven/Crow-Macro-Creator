...
def listFiles():
    ...
    goodFiles = []
    print('enter the following number to open the corresponding file')
    print('KEY\t  FILE')
...
def on_press(key):
    ...
    for i in range(len(hotkeys)):
        if(str(key) == hotkeys[i]):
            if(i == 0):#enable
                runningScript = True
                print('running script = ', runningScript)
            if(i == 1):#disable
                runningScript = False
                print('running script = ', runningScript)
            if(i == 2):#close
            ...
            if(i == 3):
                ...
                runningScript = False
                changeCsvInput = True
                print('Press a key 0 through 9 to change to another file.')
    #blocks it from typing in the chat
    if(key == Key.enter):
        runningScript = False
        print('chatting detected, script turned off')
    for readyCheck in readychecks:
        if(checkThis(readyCheck, key)):
            time.sleep(0.1)
            runningScript = True
            print('Ready! Running script = ', runningScript)
    #commands
    if(runningScript):
...
#for proper execution on .exe, need a main thread that does nothing and a running thread
